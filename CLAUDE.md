...
-   Every fork-owned file in `src/` states `Hirebotics file, not part of upstream TypeORM.` in its header.
...
-   Judge relationships, not items in isolation:
    -   identifiers that form pairs
    -   logic duplicated across files
    -   callers across files
    -   consumers of every output
    -   lifecycle symmetry (acquire/release, open/close).
-   Every local edit gets a follow-up question:
    -   what else in the system referenced, mirrored, or depended on the thing just changed?
-   A finding the user can spot in one glance at the whole file means the review was too narrow, not too strict.
...
-   Identifiers created as a pair mirror each other's structure (`TRANSACTION_BEGIN_STATEMENT` / `TRANSACTION_END_STATEMENT`).
-   Names state purpose in plain domain words. No invented jargon (banned examples: "slot", "on the wire", "load-bearing", "vacuously", "barge").
...
-   async/await over .then/.catch. Exception: tests that record promise resolution order need .then, with a one-line reason comment.
